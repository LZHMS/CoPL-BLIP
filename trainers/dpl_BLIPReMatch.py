...
class DPL(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        labelD = torch.tensor(w_expanded[undecided_ID] >= 0.5, dtype=torch.long).squeeze() #torch.tensor(undecided_gt_label == undecided_label, dtype=torch.long)  # Prompts_Num * Samples_Num * 2
        ...
        if match_probability.shape[0] > 1:
            ...
            match_ID = w >= 0.5
            for i, j, k, m in zip(undecided_label, undecided_gt_label, match_probability, w):
                self.w_epoch.append([i, j, k, m])
        else:
            match_ID = match_probability > 0.2
            self.w_epoch.append([undecided_label[0], undecided_gt_label[0], match_probability[0], match_probability[0]])
        ...
        if undecided_gt_label.shape[0] == 0:
            acc_u0, acc_u1 = 0, 0
        else:
            acc_u0 = sum(undecided_label == undecided_gt_label) / undecided_gt_label.shape[0]
            acc_u1 = sum(labels_u == undecided_gt_label) / undecided_gt_label.shape[0]
        

        all_logits_G.append(undecided_logits)
        all_labels_G.append(labels_u)
        all_gt_label.append(undecided_gt_label)
        ...
        print(logitsAll.shape)
        match_probability = logitsAll[:, 1]
        print(match_probability.shape)
        # check the samples
        if match_probability.shape[0] > 1:
            # fit a two-component GMM to the prediction loss
            match_probability = match_probability.reshape(-1, 1)
        ...
        if sum(match_ID) == 0:
            acc_rate0, acc_rate1 = 0, 0
        else:
            acc_rate0 = sum(all_labels_G == all_gt_label) / all_labels_G.shape[0]
            acc_rate1 = sum(all_labels_G[match_ID] == all_gt_label[match_ID]) / all_labels_G[match_ID].shape[0]
        print(f"undecided acc rate: {acc_u0} ===> {acc_u1}   acc rate: {acc_rate0} ===> {acc_rate1}")
    ...
    def after_epoch(self, model_id):
        ...
        if self.cfg.TRAINER.DPL.ANALYSIS and (self.epoch == 0 or (self.epoch + 1) % self.cfg.TRAINER.DPL.SAVE_INTERVAL == 0):
            w_epoch = torch.cat([torch.tensor(x).unsqueeze(0) for x in self.w_epoch])
            clean_ID = w_epoch[:, 0] == w_epoch[:, 1]
            noisy_ID = w_epoch[:, 0] != w_epoch[:, 1]
            gmm_clean_id = w_epoch[:, 3] > 0.5
            gmm_noisy_id = w_epoch[:, 3] <= 0.5
            self.visualize_samples("Match_Probs", w_epoch[clean_ID, 2], w_epoch[noisy_ID, 2])
            self.visualize_samples("GMM_Fit", w_epoch[gmm_clean_id, 3], w_epoch[gmm_noisy_id, 3])
            self.visualize_samples("GMM_Probs", w_epoch[clean_ID, 3], w_epoch[noisy_ID, 3])
