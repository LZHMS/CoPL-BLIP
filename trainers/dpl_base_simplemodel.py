...
class DPL(TrainerX):
    def __init__(self, cfg):
        ...
        self.csv_path = "output_analysis/caltech101/12_symflip/Simplemodel.csv"
        self.simple_model_epoch = {}
    ...
    def eval_train(self):
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                # label refinement
                refined_label = sharpen_prob(probs_simple, self.temp)


                # label mixrefinement
                mixrefined_predict = probs * label_onehot + (1 - probs) * probs_simple
        ...
        if (self.epoch+1) == (self.warmup_epoch+1) or (self.epoch+1) == 50 or (self.epoch+1) == 126 or (self.epoch+1) == 200:
            if "noisy id" not in self.simple_model_epoch:
                self.simple_model_epoch["noisy id"] = noisy_labels != gt_labels
            self.simple_model_epoch[f"epoch {self.epoch+1}"] = input_loss.squeeze().numpy()
            update_or_create_csv(self.simple_model_epoch, self.csv_path)
