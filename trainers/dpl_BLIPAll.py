...
class DPL(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        clean_ID = label == pred_labelG
        noisy_ID = label != pred_labelG
        # absorb clean samples
        refurbished_label[clean_ID] = label[clean_ID]
        # relabel noisy samples
        refurbished_label[noisy_ID] = robust_logits[noisy_ID].argmax(dim=1)

        # discriminator
        logitsD = self.discriminator(image, refurbished_label)     # P * B
        ...
        match_ID = (w >= 0.5) & (match_probability > 0.5).cpu().numpy().squeeze()
        for i, j, k, m in zip(refurbished_label, gt_label, match_probability, w):
            self.w_epoch.append([i, j, k, m])
        
        self.clean_samples_num += sum(match_ID)

        # save the analysed results
        if self.cfg.TRAINER.DPL.ANALYSIS:
            self.match_samples[0] += sum(label[match_ID] == gt_label[match_ID])
            self.match_samples[1] += sum(match_ID)
            self.match_samples[2] += match_probability.shape[0]

        if prec == "amp":
            with autocast():
                loss = F.cross_entropy(average_logits[match_ID], refurbished_label[match_ID])
            ...
            if self.cfg.TRAINER.DPL.USE_ROBUSTLOSS:
                lossG = self.GCE_loss(average_logits[match_ID], refurbished_label[match_ID])
                #lossD = self.GCE_loss(torch.mean(logitsD, dim=0), labelD)
            else:
                lossG = F.cross_entropy(average_logits[match_ID], refurbished_label[match_ID])
        ...
        loss_summary = {
            "lossG": lossG.item(),
            #"lossD": lossD.item(),
            "acc": compute_accuracy(average_logits[match_ID], gt_label[match_ID])[0].item(),
        }
