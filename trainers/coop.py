...
class DPL(TrainerX):
    def __init__(self, cfg):
        ...
        self.loss_epoch = {}
        self.csv_path = 'output_analysis/baseline/caltech101/2_symflip/loss.csv'
    ...
    def forward_backward(self):
        data_len = len(self.train_loader_x.dataset)
        losses = torch.zeros(data_len)  # 用于记录每个样本的损失
        is_correct = torch.zeros(data_len, dtype=torch.bool)  # 用于记录每个样本是否正确
        ...
        if prec == "amp":
            with autocast():
                for self.batch_id, batch in enumerate(self.train_loader_x):
                    ...
                    for input_i in input:
                        output_simple_i = self.model(input_i)
                        output += output_simple_i
                    output /= len(input)

                    # 计算损失
                    if self.cfg.TRAINER.DPL.GCE:
                        loss_simple = self.GCE(output, label)
                    else:
                        loss_simple = F.cross_entropy(output, label)
                    loss = loss_simple

                    # 获取预测和正确性
                    preds = torch.argmax(output, dim=1)
                    for b in range(label.size(0)):
                        index_val = index[b].item()
                        if 0 <= index_val < data_len:
                            losses[index_val] = loss.item()
                            is_correct[index_val] = (preds[b] == label[b]).item()
                ...
                if self.epoch in [2, 50, 100, 126, 150, 200]:
                    if "noisy id" not in self.loss_epoch:
                        self.loss_epoch["noisy id"] = (label != gt_label).cpu().numpy()
                    self.loss_epoch[f"epoch {self.epoch}"] = losses.cpu().numpy()
            ...
            if self.cfg.TRAINER.DPL.GCE:
                loss_simple = self.GCE(output, label.argmax(dim=1))
            else:
                loss_simple = F.cross_entropy(output, label)
