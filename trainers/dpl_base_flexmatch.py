...
class DPL(TrainerX):
            ...
            if do_test:
                if self.cfg.TEST.FINAL_MODEL == "best_val":
                    print("Deploy the model with the best val performance")
                        # self.load_model(self.output_dir, self.epoch, self.batch_idx)
                else:
                    print("Deploy the last-epoch model")
                self.test()
                self.save_model(self.epoch, self.batch_idx, self.cfg.OUTPUT_DIR)
            # self.test()
            
            # avg_perclass_accuracy = results['perclass_accuracy']
            # if best_acc < avg_perclass_accuracy:
            #     best_acc = avg_perclass_accuracy
    ...
    def save_model(
        self, epoch, batch, directory, is_best=False, val_result=None, model_name=""
    ):
