...
class DPL(TrainerX):
    ...
    def eval_train(self):
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                probs = torch.softmax(output, dim=1)
                clean_probs = ((label_clean_probs[index] + itm_probs[index]) / 2).to(self.device)

                # label refinement
                refined_label = sharpen_prob(probs, self.temp)

                # label mixrefinement
                mixrefined_predict = clean_probs * label_onehot + (1 - clean_probs) * probs / 2 
        ...
        thres = self.theta2 * (1 + self.last_epoch_num / data_len)
        match_id = torch.nonzero(match_probs >= thres, as_tuple=True)[0]
        ...
        print(f">>> samples [{total_chosen}/{data_len}] noisy rate: {noisy_rate:.2f} --> {refined_noisy_rate:.2f} --> {learned_noisy_rate:.2f} <<<")
    ...
    def forward_backward(self, batch):
        ...
        if len(input_x) > 0:
            ...
            probs = torch.softmax(output, dim=1)
            regular = -torch.mean(torch.sum(probs.log() * probs, dim=1))
            ...
            if self.cfg.TRAINER.DPL.GCE:
                loss = self.GCE(output, label_x.argmax(dim=1))
            else:
                loss = F.cross_entropy(output, label_x)
            loss = loss + self.alpha2 * penalty
            self.model_backward_and_update(loss)

            loss_summary = {
                "loss": loss.item(),
                "acc": compute_accuracy(output, label_x.argmax(dim=1))[0].item(),
            }
        else:
            loss_summary = {
                "loss": 0,
    ...
    def forward_backward_warmup(self, batch):
        ...
        if prec == "amp":
            ...
            output = self.model(input)
            loss = F.cross_entropy(output, label) + self.alpha1 * negloss(output)
            self.model_backward_and_update(loss)
