...
class DPL(TrainerX):
    ...
    def eval_train(self):
        ...
        itm_probs = torch.zeros((data_len, 1))
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                for input_i in input:
                    ...
                    itm_prob_i = self.blip(input_i, label)
        ...
        match_mean = gmm.means_.reshape(-1)
        match_std = gmm.covariances_.reshape(-1)
        idx_match = match_mean.argmax()
        idx_dismatch = match_mean.argmin()
        # from IPython import embed
        # embed()
        try:
            match_mean_ = torch.tensor(match_mean[idx_match]).cuda()
            dismatch_mean_ = torch.tensor(match_mean[idx_dismatch]).cuda()
            match_std_ = torch.tensor(match_std[idx_match]).cuda()
            dismatch_std_ = torch.tensor(match_std[idx_dismatch]).cuda()
        except:
            print(gmm.means_)
            print(idx_match)
            print(idx_dismatch)
        ...
        A = 1 / (2 * match_std_**2) - 1 / (2 * dismatch_std_**2)
        B = dismatch_mean_ / (dismatch_std_ ** 2) - match_mean_ / (match_std_ ** 2)
        C = (match_mean_ ** 2) / (2 * match_std_ ** 2) - (dismatch_mean_ ** 2) / (2 * dismatch_std_ ** 2) + torch.log(dismatch_std_ / match_std_)
        discriminant = B ** 2 - 4 * A * C


        if discriminant >= 0:
            x_1 = (-B + torch.sqrt(discriminant)) / (2 * A)
            x_2 = (-B - torch.sqrt(discriminant)) / (2 * A)
            thres1 = max(x_1, x_2).item()
        else:
            thres1 = torch.mean(torch.tensor([match_mean_, dismatch_mean_]))

        
        # divide the pseudo labels into match and unmatch set
        # preb_1 = (input_match_prob.squeeze() > thres1)
        match_probs = torch.tensor(probs[:, idx_match], dtype=torch.float)

        # thres2 = self.theta2 * (1 + self.last_epoch_num / data_len)
        # preb_2 = (match_probs >= thres1)
        # preb =  preb_1 & preb_2

        match_id = torch.nonzero(match_probs > thres1, as_tuple=True)[0]
        match_id_set = set(match_id.tolist())
        
        new_samples_id = list(match_id_set)  #list(match_id_set - self.chosen_id)
        exist_samples_id = list(self.chosen_id) #list(self.chosen_id - match_id_set)
        ...
        print(f">>> samples [{total_chosen}/{data_len}] thres: {thres1:.2f} noisy rate: {noisy_rate:.2f} --> {refined_noisy_rate:.2f} --> {learned_noisy_rate:.2f} <<<")
