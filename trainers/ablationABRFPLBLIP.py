...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, class_features=None):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.DPL.N_CTX
        ctx_init = cfg.TRAINER.DPL.CTX_INIT if class_features is None else "a photo of a"
        ...
        if class_features is None:
            ...
            prompts = [prompt_prefix + " " + name + ", " + class_features[name] + "." for name in classnames]
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, class_features=None):
        super().__init__()
        self.prompt_learner = PromptLearner(cfg, classnames, clip_model, class_features)
...
class DPL(TrainerX):
    def __init__(self, cfg):
        ...
        self.matched_refined_noisy_rates = []
        self.unmatched_refined_noisy_rates = []
    ...
    def build_model(self):
        ...
        class_features = self.dm.dataset.features
        ...
        self.featured_model = CustomCLIP(cfg, classnames, clip_model, class_features)
        self.blip = CustomBLIP(cfg, classnames, blip_model, class_features)

        print("Turning off gradients in both the image and the text encoder")
        print("The params need to be learned in simple model:")
        for name, param in self.model.named_parameters():
            if "prompt_learner" not in name:
                param.requires_grad_(False)
            if param.requires_grad:
                print(name)
        print("The params need to be learned in featured model:")
        for name, param in self.featured_model.named_parameters():
            if "prompt_learner" not in name:
                param.requires_grad_(False)
            if param.requires_grad:
                print(name)
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            ...
            load_pretrained_weights(self.featured_model.prompt_learner, cfg.MODEL.INIT_WEIGHTS)

        self.model.to(self.device)
        self.featured_model.to(self.device)
        ...
        self.featured_optim = build_optimizer(self.featured_model.prompt_learner, cfg.OPTIM)
        self.featured_sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("featured_prompt_learner", self.featured_model.prompt_learner, self.featured_optim, self.featured_sched)
        ...
        if device_count > 1:
            ...
            self.featured_model = nn.DataParallel(self.featured_model)
    ...
    def after_train(self):
        ...
        print(f"* matched noise rate: {self.matched_refined_noisy_rates}")
        print(f"* unmatched noise rate: {self.unmatched_refined_noisy_rates}")
    ...
    def warmup(self):
        ...
        #self.match_ID = self.eval_warmup()
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                for input_i in input:
                    itm_score += self.blip(input_i, label)
                itm_score /= len(input)
    ...
    def eval_train(self):
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                for input_i in input:
                    ...
                    output_featured_i = self.featured_model(input_i)
                ...
                probs_featured = F.softmax(output_featured, dim=1)
        ...
        alpha_1 = mean_clean + torch.sqrt(-2 * (std_clean ** 2) * torch.log(self.theta1 * 
            std_clean * torch.sqrt(torch.tensor(2 * torch.pi)) + 1e-8))
        alpha_2 = mean_noise - torch.sqrt(-2 * (std_noise ** 2) * torch.log(self.theta2 *
            std_noise * torch.sqrt(torch.tensor(2 * torch.pi)) + 1e-8))
	
        print(f">>> alpha1: {alpha_1:.3f}  alpha2: {alpha_2:.3f} <<<")
        if alpha_1 > alpha_2:
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                output_simple /= len(input)
                output_featured /= len(input)
                output_simple = F.softmax(output_simple, dim=1)
                output_featured = F.softmax(output_featured, dim=1)
        ...
        self.matched_refined_noisy_rates.append(round(matched_refined_noisy_rate.item(), 2))
        self.unmatched_refined_noisy_rates.append(round(unmatched_refined_noisy_rate.item(), 2))

        match_ID = torch.nonzero(match_ID, as_tuple=True)[0]
        return match_ID.cpu(), refined_labels.cpu(), refined_labels_expand.cpu()
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output_simple = self.model(all_inputs)
            output_featured = self.featured_model(all_inputs)
    ...
    def forward_backward_warmup(self, batch):
        ...
        for i, id in enumerate(index):
            if id.item() in self.match_ID:
                ...
                input_u.append(input[i])
                label_u.append(label[i])
        
        if len(input_x) > 0:
            input_x = torch.stack(input_x, dim=0)
            label_x = create_onehot(torch.tensor(label_x), self.num_classes)

        if len(input_u) > 0:
            input_u = torch.stack(input_u, dim=0)
            label_u = create_onehot(torch.tensor(label_u), self.num_classes)
    ...
    def forward_backward_x(self, batch):
        ...
        input_x = torch.stack(input_x, dim=0)
        label_x = torch.tensor(label_x).to(self.device)
        ...
        if prec == "amp":
            ...
            output_simple = self.model(input_x)
            output_featured = self.featured_model(input_x)
            loss_simple = F.cross_entropy(output_simple, label_x)
            loss_featured = F.cross_entropy(output_featured, label_x)
            loss = loss_simple + loss_featured
            self.model_backward_and_update(loss)

        loss_summary = {
            "loss": loss.item(),
            "acc": compute_accuracy(output_simple, label_x)[0].item(),
        }
    ...
    def forward_backward_mixmatch(self, batch):
        ...
        label_onehot = [label_onehot] * len(input)
        all_labels = torch.cat(label_onehot, 0)
        all_inputs = torch.cat(input, 0)
        ...
        if prec == "amp":
            with autocast():
                ...
                if self.cfg.TRAINER.DPL.GCE:
                    loss_simple = self.GCE(output_simple, mixed_label.argmax(dim=1))
                else:
            ...
            if self.cfg.TRAINER.DPL.GCE:
                ...
                loss_simple = F.cross_entropy(output_simple, mixed_label)
            loss = loss_simple #+ regular_simple
            self.model_backward_and_update(loss)

        loss_summary = {
            "loss": loss.item(),
            "acc": compute_accuracy(all_inputs, all_labels.argmax(dim=1))[0].item(),
        }
    ...
    def model_inference(self, input):
        return (self.model(input) + self.featured_model(input)) / 2
