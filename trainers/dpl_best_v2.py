...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        clean_init = cfg.TRAINER.DPL.CLEAN_INIT
        noisy_init = cfg.TRAINER.DPL.NOISY_INIT
        ...
        if clean_init:
            ...
            clean_init = clean_init.replace("_", " ")
            n_ctx = len(clean_init.split(" "))
            clean_prompt = clip.tokenize(clean_init)
            noisy_prompt = clip.tokenize(noisy_init)
            with torch.no_grad():
                clean_embedding = clip_model.token_embedding(clean_prompt).type(dtype)
                noisy_embedding = clip_model.token_embedding(noisy_prompt).type(dtype)
            clean_ctx_vectors = clean_embedding[0, 1 : 1 + n_ctx, :]
            noisy_ctx_vectors = noisy_embedding[0, 1 : 1 + n_ctx, :]
            clean_prompt_prefix = clean_init
            noisy_prompt_prefix = noisy_init
        ...
        print(f'Initial clean context: "{clean_prompt_prefix}"')
        print(f'Initial noisy context: "{noisy_prompt_prefix}"')
        print(f"Number of context words (tokens): {n_ctx}")

        self.clean_ctx = nn.Parameter(clean_ctx_vectors)  # to be optimized
        self.noisy_ctx = nn.Parameter(noisy_ctx_vectors)  # to be optimized

        classnames = [name.replace("_", " ") for name in classnames]
        name_lens = [len(_tokenizer.encode(name)) for name in classnames]
        clean_prompts = [clean_prompt_prefix + " " + name + "." for name in classnames]
        noisy_prompts = [noisy_prompt_prefix + " " + name + "." for name in classnames]

        tokenized_prompts = torch.cat([clip.tokenize(p) for prompts in [clean_prompts, noisy_prompts] for p in prompts])
        print(f"Double Prompts: {tokenized_prompts.shape}")
        with torch.no_grad():
    ...
    def forward(self):
        clean_ctx = self.clean_ctx
        noisy_ctx = self.noisy_ctx
        if clean_ctx.dim() == 2:
            clean_ctx = clean_ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
            noisy_ctx = noisy_ctx.unsqueeze(0).expand(self.n_cls, -1, -1)
        ctx = torch.cat([clean_ctx, noisy_ctx], dim=0)
...
class DPL(TrainerX):
    ...
    def build_model(self):
        ...
        self.modelA = CustomCLIPMatrix(cfg, classnames, clip_model)     # classifier
        self.modelB = CustomCLIP(cfg, classnames, clip_model)      # discriminator
        self.n_cls = self.modelA.prompt_learner.n_cls
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            load_pretrained_weights(self.modelA.prompt_learner, cfg.MODEL.INIT_WEIGHTS)
            load_pretrained_weights(self.modelB.prompt_learner, cfg.MODEL.INIT_WEIGHTS)

        self.modelA.to(self.device)
        self.modelB.to(self.device)
        # NOTE: only give prompt_learner to the optimizer
        self.optimA = build_optimizer(self.modelA.prompt_learner, cfg.OPTIM)
        self.schedA = build_lr_scheduler(self.optimA, cfg.OPTIM)
        self.register_model("prompt_learnerA", self.modelA.prompt_learner, self.optimA, self.schedA)

        self.optimB = build_optimizer(self.modelB.prompt_learner, cfg.OPTIM)
        self.schedB = build_lr_scheduler(self.optimB, cfg.OPTIM)
        self.register_model("prompt_learnerB", self.modelB.prompt_learner, self.optimB, self.schedB)
        ...
        if device_count > 1:
            ...
            self.modelB = nn.DataParallel(self.modelB)
    ...
    def forward_backward(self, batch):
        ...
        logits_matrix, _, _ = self.modelA(image)
        logitsA = torch.mean(logits_matrix, dim=0).squeeze().to(self.device)
        logitsB, _, _ = self.modelB(image)

        # produce confident label for B
        labelB = label.clone()
        index = logitsA.argmax(dim=1) != label
        labelB[index] = label[index] + self.n_cls

        # produce confident label for A
        clean_probabilities_all_classes = logitsB[:, :self.n_cls] - logitsB[:, self.n_cls:]
        labelA = label.clone()
        beta = self.cfg.TRAINER.DPL.BETA
        for i in range(labelA.shape[0]):
            labels_x = torch.zeros((1, self.n_cls), device=label.device).scatter_(1, label[i].view(-1,1), 1)
            if(logitsB[i].argmax() < self.n_cls):    # clean samples
                logits = logitsB[i, :self.n_cls] * beta + labels_x * (1-beta)      
                labelA[i] = logits.argmax()
            else:                                    # noisy samples
                negative_class = logitsB[i].argmax() - self.n_cls
                logits = torch.cat([logitsB[i, :negative_class],
                                    logitsB[i, negative_class:self.n_cls]], dim=0) 
                logits = logits    
                labelA[i] = logits.argmax()

        if prec == "amp":
            with autocast():
                loss = F.cross_entropy(logitsA, label)
            ...
            if self.cfg.TRAINER.DPL.USE_ROBUSTLOSS:
                lossA = self.GCE_loss(logitsA, labelA)
                lossB = self.GCE_loss(logitsB, labelB)
            else:
                lossA = F.cross_entropy(logitsA, labelA)
                lossB = F.cross_entropy(logitsB, labelB)
        self.model_backward_and_update(lossA, "prompt_learnerA")
        self.model_backward_and_update(lossB, "prompt_learnerB")

        loss_summary = {
            "lossA": lossA.item(),
            #"ortho_loss": self.modelA.ortho_loss.item(),
            "lossB": lossB.item(),
            "acc": compute_accuracy(logitsA, gt_label)[0].item(),
        }
