...
class DPL(TrainerX):
    def __init__(self, cfg):
        ...
        self.csv_path = "output_analysis/caltech101/2_symflip/BLIPScore.csv"
        self.blip_epoch = {}
    ...
    def eval_train(self):
        ...
        if (self.epoch+1) == (self.warmup_epoch+1) or (self.epoch+1) == 50 or (self.epoch+1) == 100 or (self.epoch+1) == 126 or (self.epoch+1) == 150 or (self.epoch+1) == 200:
            if "noisy id" not in self.blip_epoch:
                self.blip_epoch["noisy id"] = noisy_labels != gt_labels
            self.blip_epoch[f"epoch {self.epoch+1}"] = input_match_prob.squeeze().numpy()
            update_or_create_csv(self.blip_epoch, self.csv_path)
