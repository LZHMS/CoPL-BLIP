...
class DPL(TrainerX):
    ...
    def eval_train(self):
        ...
        predict_labels = torch.zeros(data_len, dtype=torch.long)
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                for b in range(label.size(0)):
                    ...
                    predict_labels[index[b]] = probs_simple[b].argmax()
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                # label refinement
                refined_label = sharpen_prob(output_simple, self.temp)

                # label mixrefinement
                mixrefined_predict = probs * label_onehot + (1 - probs) * output_simple
        ...
        # print(f">>> scores for matched samples:")
        # print(input_match_prob[noisy_labels == gt_labels].reshape(1, -1).tolist())
        # print(f">>> scores for unmatched samples:")
        # print(input_match_prob[noisy_labels != gt_labels].reshape(1, -1).tolist())
        # prob = torch.tensor(w).reshape(-1, 1)
        # refined_labels_unmatched = prob[~match_ID] * refined_labels_expand[~match_ID] + (1 - prob[~match_ID]) * create_onehot(noisy_labels[[~match_ID]], self.num_classes)
        # refined_labels_expand[~match_ID] = sharpen_prob(refined_labels_unmatched, self.temp)
        # refined_labels[~match_ID] = refined_labels_expand[~match_ID].argmax(dim=1)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            if self.cfg.TRAINER.DPL.GCE:
                ...
                loss_simple = F.cross_entropy(output_simple, all_labels)
            self.model_backward_and_update(loss_simple)
    ...
    def forward_backward_x(self, batch):
        input, label, _, _, _, _ = self.parse_batch(batch)
