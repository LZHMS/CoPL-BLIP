...
class DPL(TrainerX):
    ...
    def run_epoch(self):
        ...
        # co-divide
        if self.epoch % 5 == 0:
            self.clean_ID, self.noisy_ID, self.confused_ID, self.clean_prob = self.eval_train()
    ...
    def eval_train(self):
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                loss = F.cross_entropy(output_simple, label, reduction='none')
        ...
        if alpha_1 > alpha_2:
            clean_ID = (input_loss < alpha_2.item()) #& (predict_label == all_label).reshape(-1, 1)
            noisy_ID = (input_loss > alpha_1.item()) #& (predict_label != all_label).reshape(-1, 1)
            #print(f"clean_ID: {sum(input_loss < alpha_2.item())} clean_true: {sum(clean_ID)}")
            #print(f"noisy_ID: {sum(input_loss > alpha_1.item())} noisy_true: {sum(noisy_ID)}")
        else:
            clean_ID = (input_loss < alpha_1.item()) #& (predict_label == all_label).reshape(-1, 1)
            noisy_ID = (input_loss > alpha_2.item()) #& (predict_label != all_label).reshape(-1, 1)
            #print(f"clean_ID: {sum(input_loss < alpha_1.item())} clean_true: {sum(clean_ID)}")
            #print(f"noisy_ID: {sum(input_loss > alpha_2.item())} noisy_true: {sum(noisy_ID)}")
        ...
        return clean_ID, noisy_ID, confused_ID, clean_prob
    

    def forward_backward(self, batch):
        image, label, index, _, label_onehot = self.parse_batch(batch)

        # simple prompt learning
        output_simple = self.model(image)
        predict_simple = F.softmax(output_simple, dim=1)

        probs = self.clean_prob[index]
        # label refinement
        refined_predict = predict_simple
        refined_label = sharpen_prob(refined_predict, self.temp)

        # label mixrefinement
        mixrefined_predict = probs * label_onehot + (1 - probs) * predict_simple
        mixrefined_label = sharpen_prob(mixrefined_predict, self.temp)

        refined_labels_expand = torch.zeros((len(image), self.num_classes))
        for i, id in enumerate(index):
            if id.item() in self.clean_ID:
                # Label absorb of labeled samples
                refined_labels_expand[i] = label_onehot[i]
            elif id.item() in self.noisy_ID:
                # Label refinement for unlabeled data
                refined_labels_expand[i] = refined_label[i]
            else:
                # mixrefine confused samples
                refined_labels_expand[i] = mixrefined_label[i]
        ...
        input_a, input_b = image, image[idx]
        label_a, label_b = refined_labels_expand, refined_labels_expand[idx]
        ...
        if prec == "amp":
            ...
            if self.cfg.TRAINER.DPL.GCE:
                loss = self.GCE(output_simple, all_labels)
            else:
                loss = F.cross_entropy(output_simple, all_labels)
