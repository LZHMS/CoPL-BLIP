...
from bert.xbert import BertConfig, BertModel
from bert.vit import VisionTransformer
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, bert_model, device):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.DPL.N_CTX
        ctx_init = cfg.TRAINER.DPL.CTX_INIT
        self.tokenizer = BertTokenizer.from_pretrained(cfg.TRAINER.DPL.TEXT_ENCODER)
        self.device = device
        self.embeddings = bert_model.embeddings.to(device)

        if ctx_init:
            # use given words to initialize context vectors
            ctx_init = ctx_init.replace("_", " ")
            n_ctx = len(ctx_init.split(" "))
            ctx_prompt = self.tokenizer(ctx_init, add_special_tokens=True, padding='longest', max_length=77, return_tensors="pt")
            
            with torch.no_grad():
                input_ids = ctx_prompt.input_ids.to(device)
                input_shape = input_ids.size()
                token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
                ctx_embedding = self.embeddings(
                    input_ids=input_ids,
                    token_type_ids=token_type_ids
                )

            ctx_vectors = ctx_embedding[0, 1 : 1 + n_ctx, :]
        ...
        ctx_prompts = [prompt_prefix + " " + name + " with right label." for name in classnames]

        tokenized_prompts = self.tokenizer(ctx_prompts, add_special_tokens=True, padding='longest', max_length=77, return_tensors="pt")
        with torch.no_grad():
            input_ids = tokenized_prompts.input_ids.to(device)
            input_shape = input_ids.size()
            token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
            embedding = self.embeddings(        # cls, vocab_size, dim
                input_ids=input_ids,
                token_type_ids=token_type_ids
            )
...
class CustomBERT(nn.Module):
    def __init__(self, cfg, classnames, clip_model, text_encoder, device):
        super().__init__()
        print("Loading the Bert TextEncoder...")
        bert_config = BertConfig.from_json_file(cfg.TRAINER.DPL.BERT_CONFIG)
        self.text_encoder = BertModel.from_pretrained(text_encoder, config=bert_config, add_pooling_layer=False)       
        print("Having loaded the Bert TextEncoder")
        self.prompt_learner = PromptLearner(cfg, classnames, self.text_encoder, device)
    ...
    def forward(self, image, refurbished_label):
        _, image_features = self.image_encoder(image.to(self.dtype))
        ...
        prompts = self.prompt_learner()  # n_cls, *, ctx_dim
        refurbished_prompts = prompts[refurbished_label, :, :]
        #print(f"refurbished_prompts shape: {refurbished_prompts.shape}")

        #print("refurbished_prompts.dtype: ", refurbished_prompts.dtype)
        #print("image_features.dtype", image_features.dtype)
        #print("image_atts.dtype", image_atts.dtype)
        output = self.text_encoder(encoder_embeds=refurbished_prompts,
                            encoder_hidden_states = image_features.to(torch.float32),
                            encoder_attention_mask = image_atts,        
                            return_dict = True
                            )
        prediction = self.cls_head(output.last_hidden_state[:, 0, :])  # B * C
        return prediction
...
class CustomCLIPMatrix(nn.Module):
    ...
    def forward(self, image):
        clip_image_features, _ = self.image_encoder(image.type(self.dtype))
    ...
    # def ortho_penalty(self, prompts):
    #     loss = 0
    #     for word in range(prompts.shape[2]):
    #         t = prompts[:, 0, word, :]
    #         loss += ((t @t.T - torch.eye(t.shape[0]).cuda())**2).mean()
    #     return loss/10
...
class DPL(TrainerX):
    ...
    def build_model(self):
        ...
        self.discriminator = CustomBERT(cfg, classnames, clip_model, cfg.TRAINER.DPL.TEXT_ENCODER, self.device)      # discriminator
    ...
    def forward_backward(self, batch):
        ...
        clean_samples_ID_G = w >= 0.5     # the lower mean loss
        noisy_samples_ID_G = w < 0.5    # the higher mean loss

        # refurbish the pseudo label
        refurbished_label = label.clone()
        labels_x = torch.zeros((label.shape[0], self.n_cls), device=label.device).scatter_(1, label.view(-1,1), 1)
        ...
        refurbished_label[clean_samples_ID_G] = robust_logits[clean_samples_ID_G].argmax(dim=1)
        # noisy samples
        refurbished_label[noisy_samples_ID_G] = average_logits[noisy_samples_ID_G].argmax(dim=1)

        # discriminator
        logitsD = self.discriminator(image, refurbished_label)     # B * 2
        # calculate the max similarity for the specific two prompts
        probsD = F.softmax(logitsD, dim=1)
        clean_probability = probsD[:, 1]
        #clean_probabilities_all_classes = logitsD[:, :self.n_cls] - logitsD[:, self.n_cls:]
        
        # divide the noisy dataset by discriminator
        beta = 0.4
        clean_samples_ID_D = clean_probability >= beta     # the lower mean loss
        noisy_samples_ID_D = clean_probability < beta    # the higher mean loss

        # produce label for Generator
        labelG = refurbished_label.clone()
        # clean samples passed the check of Discriminator so just need to update the labels of noisy samples
        # noisy samples
        #mask = torch.full_like(clean_probabilities_all_classes, float('-inf'))
        #clean_probabilities_all_classes.scatter_(1, refurbished_label.unsqueeze(1), mask)
        #labelG[noisy_samples_ID_D] = torch.argmax(clean_probabilities_all_classes[noisy_samples_ID_D, :], dim=1)
        #labelG[noisy_samples_ID_D] = label[noisy_samples_ID_D]   # disgard the lower quality samples
        # print(average_logits.shape)
        # print(average_logits[clean_samples_ID_D, :].shape)
        # print(labelG[clean_samples_ID_D].shape)
        # print(average_logits[clean_samples_ID_D, :])
        # print(labelG[clean_samples_ID_D])
        #self.beta = self.cfg.TRAINER.DPL.BETA * (1 + sum(clean_samples_ID_D) / clean_probability.shape[0])
        # produce label for Discriminator
        labelD = torch.tensor(clean_samples_ID_G).long().clone().to(self.device)

        if prec == "amp":
            with autocast():
                loss = F.cross_entropy(average_logits, labelG)
            ...
            if self.cfg.TRAINER.DPL.USE_ROBUSTLOSS:
                lossG = self.GCE_loss(average_logits[clean_samples_ID_D, :], labelG[clean_samples_ID_D])
                lossD = self.GCE_loss(logitsD, labelD)
            else:
                lossG = F.cross_entropy(average_logits[clean_samples_ID_D, :], labelG[clean_samples_ID_D])
                lossD = F.cross_entropy(logitsD, labelD)
        ...
        loss_summary = {
            "lossG": lossG.item(),
            "lossD": lossD.item(),
            "acc": compute_accuracy(average_logits, gt_label)[0].item(),
        }
