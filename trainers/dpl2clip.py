...
class DPL(TrainerX):
    ...
    def build_model(self):
        ...
        self.model_A = CustomCLIP(cfg, classnames, clip_model)
        self.model_B = CustomCLIP(cfg, classnames, clip_model)

        print("Turning off gradients in both the image and the text encoder")
        for name, param in self.model_A.named_parameters():
            if "prompt_learner" not in name:
                param.requires_grad_(False)
        for name, param in self.model_B.named_parameters():
            if "prompt_learner" not in name:
                param.requires_grad_(False)

        if cfg.MODEL.INIT_WEIGHTS:
            load_pretrained_weights(self.model_A.prompt_learner, cfg.MODEL.INIT_WEIGHTS)

        self.model_A.to(self.device)
        self.model_B.to(self.device)
        # NOTE: only give prompt_learner to the optimizer
        self.optim = build_optimizer(self.model_A.prompt_learner, cfg.OPTIM)
        self.sched = build_lr_scheduler(self.optim, cfg.OPTIM)
        self.register_model("prompt_learner_A", self.model_A.prompt_learner, self.optim, self.sched)

        self.foptim = build_optimizer(self.model_B.prompt_learner, cfg.OPTIM)
        self.fsched = build_lr_scheduler(self.foptim, cfg.OPTIM)
        self.register_model("prompt_learner_B", self.model_B.prompt_learner, self.foptim, self.fsched)
    ...
    def eval_train(self):
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                for input_i in input:
                    output_simple_i = self.model_A(input_i)
                    output_featured_i = self.model_B(input_i)
        ...
        with torch.no_grad():
            for self.batch_id, batch in enumerate(self.train_loader_x):
                ...
                clean_probs = label_clean_probs[index].to(self.device)
        ...
        self.refined_labels_expand = refined_labels_expand
        ...
        print(f">>> noisy rate: {noisy_rate:.2f} --> {refined_noisy_rate:.2f} <<<")
    ...
    def forward_backward(self, batch):
        ...
        if len(input_x) > 0:
            ...
            output_simple = self.model_A(mixed_input)
            output_featured = self.model_B(mixed_input)
    ...
    def forward_backward_warmup(self, batch):
        ...
        if prec == "amp":
            ...
            output_simple = self.model_A(input)
            output_featured = self.model_B(input)
