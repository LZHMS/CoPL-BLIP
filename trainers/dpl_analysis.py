...
class DPL(TrainerX):
    ...
    def eval_train(self):
        ...
        print(f">>> theta: {self.theta} clean: {len(clean_ID)} noisy: {len(noisy_ID)} confused: {len(confused_ID)} <<<")
        ...
        num_clean_samples = len(clean_ID)
        num_noisy_samples = len(noisy_ID)
        num_confused_samples = len(confused_ID)
        ...
        if num_clean_samples > 0:
            clean_ratio_in_clean = clean_in_clean / num_clean_samples
        else:
            clean_ratio_in_clean = 0

        if num_noisy_samples > 0:
            clean_ratio_in_noisy = clean_in_noisy / num_noisy_samples
        else:
            clean_ratio_in_noisy = 0

        if num_confused_samples > 0:
            clean_ratio_in_confused = clean_in_confused / num_confused_samples
        else:
            clean_ratio_in_confused = 0

        # 输出统计结果
        print("Clean samples in Clean set:", clean_in_clean, "/", num_clean_samples)
        print("Noisy samples in Clean set:", noise_in_clean, "/", num_clean_samples)
        print("Clean samples in Noisy set:", clean_in_noisy, "/", num_noisy_samples)
        print("Noisy samples in Noisy set:", noise_in_noisy, "/", num_noisy_samples)
        print("Clean samples in Confused set:", clean_in_confused, "/", num_confused_samples)
        print("Noisy samples in Confused set:", noise_in_confused, "/", num_confused_samples)

        print("Clean ratio in Clean set:", clean_ratio_in_clean)
        print("Clean ratio in Noisy set:", clean_ratio_in_noisy)
        print("Clean ratio in Confused set:", clean_ratio_in_confused)
        ...
        # divide the pseudo labels into good sample and bad sample
    ...
    def forward_backward_clean(self, batch):
        input, label, index, _, _, _ = self.parse_batch(batch)
        ...
        match_empty = len(input_x) == 0
        if not match_empty:
            all_inputs = torch.stack(input_x, dim=0).to(self.device)
            all_labels = torch.stack(label_x, dim=0).to(self.device)
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output = self.bothmodel_inference(input)
    ...
    def bothmodel_inference(self, input):
